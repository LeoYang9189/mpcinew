...
interface AFRRecord {
    // 系统字段
    id: string
    status: string // AFR Status: SPD, HLD, DNU, Pending, Draft
    creator: string
    createTime: string
    updater: string
    updateTime: string
    ...
    naccsCode: string // NACCS Carrier Code
    // 航次信息
    vessel: string
    voyage: string
    pol: string // Port of Loading
    pod: string // Port of Discharge (Japan)
    etd: string
    eta: string
    // 货物信息
    qty: number
...
const StatusBadge = ({ status }: { status: string }) => {
    const styles: Record<string, string> = {
        "SPD": "bg-emerald-50 text-emerald-700 ring-emerald-600/20",
        "HLD": "bg-red-50 text-red-700 ring-red-600/20",
        "DNU": "bg-amber-50 text-amber-700 ring-amber-600/20",
...
const ActionButtons = ({
    ...
    <div className="flex items-center gap-0.5">
        ...
        <ActionButton
            ...
            onClick={() => onEdit(id)}
        />
        <ActionButton icon={Send} label="发送AFR" className="hover:bg-teal-50 text-slate-500 hover:text-teal-600" />
        <ActionButton icon={Download} label="下载回执" className="hover:bg-slate-100 text-slate-500 hover:text-slate-700" />
    </div>
)

interface AFRListProps {
    title: string
    description: string
}
 ...
 * AFR 列表组件
 */
export default function AFRList({ title, description }: AFRListProps) {
    const navigate = useNavigate()

    const handleView = useCallback((id: string) => {
        navigate(`/dashboard/customs/afr/view/${id}`)
    }, [navigate])

    const handleEdit = useCallback((id: string) => {
        navigate(`/dashboard/customs/afr/edit/${id}`)
    }, [navigate])

    // Mock 数据配置
    const statuses = ["SPD", "HLD", "DNU", "Pending", "Draft"]
    const users = ["John Smith", "Emily Chen", "Robert Wang"]
    const pols = ["CN SHA", "CN NGB", "CN YTN", "KR PUS"]
    const pods = ["JP TYO", "JP YOK", "JP OSA", "JP KOB", "JP NGO"]
    const shippers = ["SHANGHAI TRADING", "NINGBO EXPORT", "QINGDAO LOGISTICS"]
    const consignees = ["TOKYO IMPORTS", "OSAKA TRADING", "JAPAN RETAIL"]

    // 生成 mock 数据
    const data: AFRRecord[] = useMemo(() => {
        return Array.from({ length: 50 }, (_, i) => {
            const idx = i + 1
            const status = statuses[Math.floor(Math.random() * statuses.length)]

            return {
                id: `AFR${String(idx).padStart(6, "0")}`,
                mblNo: `MBL${Math.floor(Math.random() * 90000000) + 10000000}`,
                hblNo: `HBL${Math.floor(Math.random() * 90000000) + 10000000}`,
                naccsCode: `NACS${Math.floor(Math.random() * 9000) + 1000}`,
                status,
    ...
    // 列定义
    const columns: ColumnDef<AFRRecord, unknown>[] = useMemo(
        () => [
            {
                accessorKey: "hblNo",
                header: "HBL No",
                cell: ({ row }) => <span className="font-mono font-medium">{row.getValue("hblNo")}</span>,
                size: 140,
            },
            {
                accessorKey: "status",
            ...
            {
                accessorKey: "naccsCode",
                header: "NACCS Code",
                cell: ({ row }) => <span className="font-mono">{row.getValue("naccsCode")}</span>,
                size: 100,
            },
            {
                accessorKey: "pol",
    ...
    const [selectedRows, setSelectedRows] = useState<AFRRecord[]>([])
            ...
            {viewMode === "table" ? (
                <Card className="p-4 min-w-0 flex-1 flex flex-col overflow-hidden">
                    <DataTable
                        data={data}
                        columns={columns}
                        searchPlaceholder="搜索HBL、MBL、NACCS..."
                        enableRowSelection
