...
interface ManifestRecord {
    // 系统字段
    id: string
    status: string // Manifest Status: Reported, Arrived, Released, Rejected, Draft
    creator: string
    createTime: string
    updater: string
    updateTime: string
    ...
    crn: string // Carrier Code
    manifestType: string // Highway, Air, Marine, Rail
    portOfReport: string
    eta: string
    conveyanceRef: string // Truck No, Flight No, Vessel Name
    crew: string // Driver/Crew Name
    // 货物信息
    qty: number
    weight: number
    shipper: string
...
const ActionButtons = ({
    ...
    <div className="flex items-center gap-0.5">
        ...
        <ActionButton
            ...
            onClick={() => onEdit(id)}
        />
        <ActionButton icon={Send} label="发送舱单" className="hover:bg-teal-50 text-slate-500 hover:text-teal-600" />
 ...
 * eManifest 列表组件
 */
export default function ManifestList({ title, description }: ManifestListProps) {
    const navigate = useNavigate()

    const handleView = useCallback((id: string) => {
        navigate(`/dashboard/customs/emanifest/view/${id}`)
    }, [navigate])

    const handleEdit = useCallback((id: string) => {
        navigate(`/dashboard/customs/emanifest/edit/${id}`)
    }, [navigate])

    // Mock 数据配置
    const statuses = ["Reported", "Arrived", "Released", "Rejected", "Draft"]
    const types = ["Highway", "Air", "Marine", "Rail"]
    const users = ["John Smith", "Emily Chen", "Robert Wang"]
    const ports = ["Windsor", "Sarnia", "Fort Erie", "Pacific Highway", "Emerson"]
    const shippers = ["US EXPORT INC", "GLOBAL TRADING LLC", "NORTH AMERICAN MFG"]
    const consignees = ["CANADA IMPORTS LTD", "TORONTO RETAIL", "VANCOUVER LOGISTICS"]

    // 生成 mock 数据
    const data: ManifestRecord[] = useMemo(() => {
        return Array.from({ length: 50 }, (_, i) => {
            ...
            return {
                id: `EM${String(idx).padStart(6, "0")}`,
                ccn: `CCN${String(idx).padStart(10, "0")}`,
                crn: `CRN${Math.floor(Math.random() * 9000) + 1000}`,
                manifestType: type,
                status,
                ...
                conveyanceRef: type === "Highway" ? `TRUCK${Math.floor(Math.random() * 900) + 100}` : `FLIGHT${Math.floor(Math.random() * 900) + 100}`,
                crew: "Mike Johnson",
    ...
    // 列定义
    const columns: ColumnDef<ManifestRecord, unknown>[] = useMemo(
        () => [
            {
                accessorKey: "ccn",
                header: "CCN",
            ...
            },
            {
                ...
                header: "类型",
                cell: ({ row }) => <span className="whitespace-nowrap">{row.getValue("manifestType")}</span>,
                meta: { filterVariant: "select" },
                size: 80,
            },
            {
                accessorKey: "crn",
                header: "CRN",
                cell: ({ row }) => <span className="font-mono">{row.getValue("crn")}</span>,
                size: 80,
            },
            {
    ...
    const [selectedRows, setSelectedRows] = useState<ManifestRecord[]>([])
    ...
    const pivotFields: PivotField[] = useMemo(
        () => [
            ...
            { id: "manifestType", label: "类型", type: "string" },
            ...
            {viewMode === "table" ? (
                <Card className="p-4 min-w-0 flex-1 flex flex-col overflow-hidden">
                    <DataTable
                        data={data}
                        columns={columns}
                        searchPlaceholder="搜索CCN、CRN、运输工具..."
                        enableRowSelection
                ...
                <Card className="p-4">
                    ...
                    <PivotTable
                        data={data}
                        fields={pivotFields}
                        defaultRowFields={["status", "manifestType"]}
